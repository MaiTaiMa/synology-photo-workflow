...
Datei: tests/test_manual_keep_v77.py
...
def test_manual_keep_moves_only_clear_best_of_multiple_candidates(tmp_path):
    inbox, used = tmp_path/'inbox', tmp_path/'used'; inbox.mkdir()
    source = inbox/'keep.jpg'; source.write_bytes(b's')
    first, second = tmp_path/'first.jpg', tmp_path/'second.jpg'; first.write_bytes(b'a'); second.write_bytes(b'b')
...
def test_manual_keep_keeps_ambiguous_source_in_inbox(tmp_path):
    inbox, used = tmp_path/'inbox', tmp_path/'used'; inbox.mkdir(); (inbox/'keep.jpg').write_bytes(b's')
    first, second = tmp_path/'first.jpg', tmp_path/'second.jpg'; first.write_bytes(b'a'); second.write_bytes(b'b')
