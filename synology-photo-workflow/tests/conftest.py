...
SICHERHEIT: Fixtures erzeugen ausschließlich temporäre Verzeichnisse ohne Originale oder biometrische Daten.
"""
...
def write_config(tmp_path, **overrides):
    cfg = {
        ...
        'culling': {
            ...
            'star_rating_bands': [
                {'min': 0, 'max': .19, 'rating': 0},
                {'min': .2, 'max': .39, 'rating': 1},
                {'min': .4, 'max': .59, 'rating': 2},
                {'min': .6, 'max': .74, 'rating': 3},
                {'min': .75, 'max': .89, 'rating': 4},
                {'min': .9, 'max': 1, 'rating': 5},
            ],
        },
        'phase2': {
